...
class MemriseAPI {
  constructor() {
    ...
    this.get_course_url =
      constant.MEMRISE_BASE_URL + constant.MEMRISE_GET_COURSES;
    this.bulk_add_url =
      constant.MEMRISE_BASE_URL + constant.MEMRISE_BULK_UPLOAD;
  ...
  async login() {
    try {
      const login_page = await this.agent
        .get(this.login_url)
  ...
  async scrapeCourseWords(course) {
    try {
      ...
      const kr_words = [];
      const en_words = [];
      const kr_column = $('.col_a.col.text').each(function (i, div) {
        kr_words.push($(div).text());
      });
      const en_column = $('.col_b.col.text').each(function (i, div) {
  ...
  async get_course_edit_id(url) {
    ...
    const id = this.$('.rebrand.reverse-header-ruled.level-view').attr()[
      'data-level-id'
    ];
    return id;
  }
  ...
  async upload_word_list(wordlist, course_url, voice, speed, id) {
    ...
    for (let word of wordlist) {
      ...
      if (exists === null) {
        ...
        result = await mongodb.store_tts(
          word,
          translated_word,
          voice,
          speed,
